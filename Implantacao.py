...
class Implantacao:
    ...
    def check_zd_user(self, cst_email):
        url = self.zenAPI.prod_url + self.zenAPI.search_users + cst_email
        response = self.zenAPI.get_request(url, new_instance=True)
        user = response['users']
        return user
    ...
    def create_ticket(self, cst_email):
        ...
        group_id = 27881687140627
        submitter_id = 26772636303507
        payload = {
            "ticket": {
                "subject": f"{subject}",
                "comment": {
                ...
                "is_public": "true",
                "status": "pending",
                "brand_id": self.zenAPI.brands['loja'],
    ...
    def update_tickets(self, ticket_id, comment, status, situacao_ticket, group_id):
        url = self.zenAPI.prod_url + self.zenAPI.ticekts + ticket_id
        print(url)
        # submitter_id = 24966456979859  # TROCAR PARA PROD
        submitter_id = 26772636303507
        payload = {
    ...
    def process_implantacao(self):
        ...
        if rd_tickets:
            for i in rd_tickets:
                ...
                print(ticket_id)
                cst_tkt_link = 'https://atendimentohgbrasil.zendesk.com/agent/tickets/' + ticket_id
                rd_comment = f"""Ticket aberto com o cliente: {cst_tkt_link}
                """
                rd_ticket_response = self.update_tickets(rd_ticket_id,
                                                         rd_comment,
                                                         status='pending',
                                                         situacao_ticket='dl_atendido',
                                                         group_id=rd_group_id)
                print(rd_ticket_response)
                rd_tkt_link = 'https://atendimentohgbrasil.zendesk.com/agent/tickets/' + rd_ticket_id
                cst_comment = f"""Ticket da RD: {rd_tkt_link}
                Link para dados do cliente: {rd_lead_url}
                """
                if job_status == 'completed':
                    cst_ticket_response = self.update_tickets(ticket_id,
                                                              cst_comment,
                                                              status='pending',
    ...
    def process_form_answers(self):
        ...
        while num_msgs > 0:
            ...
            if ticket_exists:
                ...
                """
                response = self.update_tickets(ticket_id,
                                               comment,
                                               status='open',
                                               situacao_ticket='dl_aguardando_consultor',
                                               group_id=26367840632851)
